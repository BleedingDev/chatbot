...
import { ErrorMessage } from "@/components/llm-stocks/message"
...
async function submitUserMessage(content: string) {
  ...
  const showStockPurchaseUiParams = z.object({
  ...
  const showStockPurchaseUi = ({ symbol, price, numberOfShares = 100 }: z.infer<typeof showStockPurchaseUiParams>) => {
  ...
  const showStockPriceParams = z.object({
    symbol: z.string().describe("The name or symbol of the stock or currency. e.g. DOGE/AAPL/USD."),
    price: z.number().describe("The price of the stock."),
    delta: z.number().describe("The change in price of the stock"),
  })
  const showStockPrice = async ({ symbol, price, delta }: z.infer<typeof showStockPriceParams>) => {
    reply.update(
      <BotCard>
        <StockSkeleton />
      </BotCard>,
  ...
  const listStocksParams = z.object({
    stocks: z.array(
      z.object({
  ...
  const listStocks = async ({ stocks }: z.infer<typeof listStocksParams>) => {
    reply.update(
      <BotCard>
        <StocksSkeleton />
      </BotCard>,
  ...
  const getEvents = async ({ events }: z.infer<typeof getEventsParams>) => {
    reply.update(
      <BotCard>
        <EventsSkeleton />
      </BotCard>,
...
Besides that, you can also chat with users and do some calculations if needed.`,
    ...
    functions: [
      {
        ...
        parameters: showStockPriceParams,
        func: showStockPrice,
      },
      {
        name: "show_stock_purchase_ui",
        ...
        parameters: showStockPurchaseUiParams,
        func: showStockPurchaseUi,
      },
      {
        name: "list_stocks",
        description: "List three imaginary stocks that are trending.",
        parameters: listStocksParams,
        func: listStocks,
      },
      {
        name: "get_events",
  ...
  completion.onError((content: string) => {
    reply.update(<ErrorMessage>{content}</ErrorMessage>)
    reply.done()
    aiState.done([...aiState.get()])
  })

  // TODO: Remove after full migration to langchain
  completion.onFunctionCall("list_stocks", listStocks)
  completion.onFunctionCall("get_events", getEvents)
  completion.onFunctionCall("show_stock_price", showStockPrice)
  completion.onFunctionCall("show_stock_purchase_ui", showStockPurchaseUi)
